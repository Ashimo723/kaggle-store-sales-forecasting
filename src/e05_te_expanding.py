"""e05: expanding(過去のみ)TE。e04(素朴TE=自己リーク版, 0.3906)の修正。他は e02 と同一（同一 params / 同一 fold / 同一ベース特徴）。
...
PARAMS = {**_P, 'num_threads': 5}  # e03 と並行実行するためスレッドを分ける
...
for k, (tr_end, vs, ve) in enumerate(folds):
    ...
    te_df, te_cols = TE.add_te(df, te_mask, level_days=LEVEL_DAYS, m=M)
...
print(f"\n== e05 TE(expanding) (start={TRAIN_START.date()}, level_days={LEVEL_DAYS}, m={M}) mean {np.mean(sc):.4f} ==")

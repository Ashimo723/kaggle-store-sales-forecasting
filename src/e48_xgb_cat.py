"""e48: XGBoost / CatBoost 脚を追加してアンサンブルする（ユーザー提案）。
...
# CatBoost 用: カテゴリは文字列に
CATCOLS = F.CATS
dcat = df.copy()
for col in CATCOLS:
    dcat[col] = dcat[col].astype(str)
...
for fname, vs, ve in cv3.FOLDS:
    ...
    # (3) CatBoost
    cm = CatBoostRegressor(depth=8, learning_rate=0.05, iterations=NR, loss_function="RMSE",
                           random_seed=42, verbose=0, thread_count=8, l2_leaf_reg=3.0)
    cm.fit(Pool(dcat.loc[m_tr, F.FEATURES], dcat.loc[m_tr, "y"], cat_features=CATCOLS))
    p_cat = np.clip(cm.predict(Pool(dcat.loc[m_va, F.FEATURES], cat_features=CATCOLS)), 0, None)
    row["cat"] = float(np.sqrt(np.mean((p_cat - yv) ** 2)))

    # 残差相関（直交性の指標）
    rl = p_lgb - yv
    row["corr_xgb"] = float(np.corrcoef(rl, p_xgb - yv)[0, 1])
    row["corr_cat"] = float(np.corrcoef(rl, p_cat - yv)[0, 1])
    ...
    for tag, p in [("xgb", p_xgb), ("cat", p_cat)]:
        for w in [0.15, 0.25, 0.35]:
            row[f"best4+{tag}{w}"] = float(np.sqrt(np.mean(((1-w)*best4 + w*p - yv) ** 2)))
    row["best4+both.30"] = float(np.sqrt(np.mean((0.7*best4 + 0.15*p_xgb + 0.15*p_cat - yv) ** 2)))
    np.savez_compressed(c.OUTPUT / f"e48_pred_{fname}.npz", xgb=p_xgb, cat=p_cat)
    rec[fname] = row
    print(f"{fname}  lgb {row['lgb']:.4f}  xgb {row['xgb']:.4f}  cat {row['cat']:.4f}  "
          f"| corr {row['corr_xgb']:.3f}/{row['corr_cat']:.3f}  best4 {row['best4']:.4f} "
          f"+xgb.25 {row['best4+xgb0.25']:.4f} +cat.25 {row['best4+cat0.25']:.4f}  {time.time()-t0:.0f}s", flush=True)
